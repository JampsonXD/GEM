...
#include "ASyncTasks/AT_PlayMontageAndWaitForEvent.h"
...
void UAT_PlayMontageAndWaitForEvent::Activate()
{
	...
	if (AbilitySystemComponent)
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("UAT_PlayMontageAndWaitForEvent call to PlayMontage failed!"));
		}
	}
	else
	{
		UE_LOG(LogTemp, Warning, TEXT("UAT_PlayMontageAndWaitForEvent called on invalid AbilitySystemComponent"));
	}

	if (!bPlayedMontage)
	{
		UE_LOG(LogTemp, Warning, TEXT("UAT_PlayMontageAndWaitForEvent called in Ability %s failed to play montage %s; Task Instance Name %s."), *Ability->GetName(), *GetNameSafe(MontageToPlay), *InstanceName.ToString());
